...
def main():
    ...
    parser.add_argument('--folder_path', type=str, default='./holo/video/',
    ...
    base_dir = '/data/vision/torralba/scratch/yichenl/projects/source/act/exp/video/'
    base_path = os.path.join(base_dir, args.folder_path, args.video_name, "Export_py")
    ...
    action_aligned_npz_path = os.path.join(base_path,"Video","action_aligned")
    hands_path = os.path.join(base_dir, args.folder_path.replace('video', 'hand-pose'), args.video_name, "Export_py", 'Hands')
    intrinsic_path = os.path.join(base_dir, args.folder_path.replace('video', 'cam_info'), args.video_name, "Export_py", "Video")
    imu_path = os.path.join(base_dir, args.folder_path.replace('video', 'imu'), args.video_name, "Export_py", "IMU")
    ...
    frame_rate, frame_num = get_framerate(os.path.join(base_path,"Video_compress.mp4"))
    img_timing_array = []
    ...
    # Read Accelerometer
    acc_path = os.path.join(imu_path, 'Accelerometer_sync.txt')
    acc_array = read_imu_txt(acc_path)
    acc_time_stamp = acc_array[:, 1]
    
    gyro_path = os.path.join(imu_path, 'Gyroscope_sync.txt')
    gyro_array = read_imu_txt(gyro_path)
    gyro_time_stamp = gyro_array[:, 1]
    
    mag_path = os.path.join(imu_path, 'Magnetometer_sync.txt')
    mag_array = read_imu_txt(mag_path)
    mag_time_stamp = mag_array[:, 1]
    ...
    while np.std(left_hand_array[start_frame][4:].reshape(3, -1)[0, :]) < 1e-5:
        start_frame += 1
    

    while np.std(left_hand_array[end_frame][4:].reshape(3, -1)[0, :]) < 1e-5:
        end_frame -= 1
    
    num_frames = len(img_sync_timing_array)

    print('effective start and end frame', start_frame, end_frame)
    
    assert num_frames == left_hand_array.shape[0] == acc_array.shape[0], f'{num_frames} {left_hand_array.shape[0]} {acc_array.shape[0]}'
    ...
    if args.save_video:
        ...
        if not os.path.exists(mpeg_img_path):
            ...
            os.makedirs(mpeg_img_path, exist_ok=True)
            os.system("ffmpeg -i ../Video_compress.mp4 -start_number 0 images/%06d.png")
        
        if not os.path.exists(mpeg_aligned_img_path):
            os.makedirs(mpeg_aligned_img_path, exist_ok=True)
            os.makedirs(action_aligned_npz_path, exist_ok=True)
            ...
            for ii in range(start_frame, num_frames+end_frame):
                _, mpeg_indices = mpeg_nbrs.kneighbors(
                    np.array(img_sync_timing_array[ii]).reshape(-1, 1))
                ...
                dest_action = os.path.join(action_aligned_npz_path,'{0:06d}.npz'.format(ii))
                shutil.copy(source , dest)
                hand_point_left = left_hand_array[ii][4:].reshape(3, -1)
                hand_point_right = right_hand_array[ii][4:].reshape(3, -1)
                acc_point = acc_array[ii][2:]
                gyro_point = gyro_array[ii][2:]
                mag_point = mag_array[ii][2:]
                np.savez(dest_action, left_hand=hand_point_left, right_hand=hand_point_right, acc=acc_point, gyro=gyro_point, mag=mag_point)
    ...
    # Read cam instrics
    img_instrics_path = os.path.join(intrinsic_path, 'Intrinsics.txt')

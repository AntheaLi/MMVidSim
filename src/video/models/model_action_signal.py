...
from exp.video.models.encoder_signals import SignalEncoder, TextEncoder, SignalFrameEncoder, SignalDecoder, SignalFrameDecoder, GlobalActivationLeakyReLULinear, LocalActivationLeakyReLULinear
...
class GlobalHyperplaneMetricLayers(nn.Module):
    ...
    def forward(self, batch):
        ...
        signals = [x for x in list(batch.keys()) if x in self.signals]
        batch_size, num_frames  = batch[signals[0]].shape[0], batch[signals[0]].shape[1]
        # Getting Image and Text Features
...
class Network(nn.Module):
    def __init__(
        ...
        elif self.hyper_plane == 'global-metric-mlp': self.signal_encoder = GlobalActivationLeakyReLULinear(args, signals=signals)
        else:

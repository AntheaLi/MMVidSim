...
class Trainer(object):
    ...
    def sample(self, batch, batch_condition, batch_size=1, guide_scale=0):
        ...
        return self.ema.ema_model.sample(image=batch.to(device), action_data=task_embeds.to(device), batch_size=batch_size, guide_scale=guide_scale)
    ...
    def eval(self):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(self.valid_dl):
                ...
                x = batch['video']
                B, T, H, W, C = x.shape
                x_feat = self.ae.encode(rearrange(x, 'b t h w c -> (b t) c h w'))
                bt, c, h, w = x.shape

                if self.model_type == 'avdc':
                    x_feat = rearrange(x_feat, '(b t) c h w -> b t c h w', b=B, t=T) 
                    b, t, c, h, w = x_feat.shape
                    x_cond = x_feat[:, 0:1]
                    x_conds_img.append(x[:, 0:1].reshape(B, 1 * C, H, W))
                    xs.append(x_feat[:, 1:].reshape(b, (t - 1) * c, h, w))
                    xs_img.append(x[:, 1:].reshape(B, (T - 1) * C, H, W))


                elif 'i2vgen' in self.model_type or 'our' in self.model_type:
                    x_feat = rearrange(x_feat, '(b t) c h w -> b c t h w', b=B, t=T) 
                    x_conds.append(x_feat[:, :, 0:1])
                    xs.append(x_feat[:, :, 1:])                    
                    x_conds_img.append(rearrange(x[:, 0:1], 'b t c h w -> b c t h w')) # for visualization
                    xs_img.append(rearrange(x[:, 1:], 'b t c h w -> b c t h w'))
            ...
            if self.model_type == 'i2vgen' and self.guide_scale > 0.0:
                ...
                with self.accelerator.autocast():
                    all_xs_list = list(map(lambda n, l, c, e: self.ema.ema_model.sample(batch_size=n, negative_task_emb=l, image=c, action_data=e), batches, negative_task_embeds, x_conds, task_embeds))  
            else:
                with self.accelerator.autocast():
                    all_xs_list = list(map(lambda n, c, e: self.ema.ema_model.sample(batch_size=n, image=c, action_data=e), batches, x_conds, task_embeds))
        

        print_gpu_utilization()
        
        gt_xs = torch.cat(xs_img, dim = 0) # [batch_size, 3*n, 120, 160]
        ...
        all_xs = self.ae.decode(all_xs).detach().cpu() # Decoder

...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        assert model_type in ['avdc', 'i2vgen', 'i2vgen-avdc', 'our', 'ours', 'ours-lcd', 'ours-i2v'], f'{model_type} not implemented'
        ...
        dl = DataLoader(self.ds, batch_size = train_batch_size, shuffle = True, pin_memory = True, num_workers = 32, collate_fn = collate_none)
        # dl = dataloader
        dl = self.accelerator.prepare(dl)
        self.dl = cycle(dl)
        self.valid_dl = DataLoader(self.valid_ds, batch_size = valid_batch_size, shuffle = True, pin_memory = True, num_workers = 32, collate_fn = collate_none)
        self.num_batches =  (len(self.ds) // train_batch_size)
        self.eval_num_steps = self.eval_freq * self.num_batches
    ...
    def encode_action(self, batch):
        if self.use_text_condition: 
            # print('label_text', batch['label_text'][0].shape)
            goal = batch['label_text']
            goal_embed = self.condition_encoder(goal)
            # goal = batch['label_text'][0]
            # goal_embed = self.condition_encoder(goal)
            # print('goal [0]', goal_embed.shape)
            ...
            if self.context_encoder is not None:
                action['context'] = self.context_encoder(batch['video'][:, 0])

...
from .modules.i2vgen.unet.unet_ours_lcd_ll import UNetSD_Our_LCD_LL
...
class VideoDiffusion(nn.Module):
    def __init__(
        ...
        if self.model_type == 'i2vgen':
            ...
            self.pipeline = GoalGaussianDiffusionLCD(self.unet, **args.video_model_config['i2vgen-avdc'])    
        elif self.model_type == 'ours-lcd-ll':
            self.unet = UNetSD_Our_LCD_LL(**args.video_model_config['our']['unet-lcd-ll'])
            self.pipeline = GoalGaussianDiffusionLCD(self.unet, **args.video_model_config['i2vgen-avdc'])    
            ...
            self.pipeline = I2VGenXLPipeline.from_pretrained(args.video_model_config['i2vgen']['version'], torch_dtype=torch.float16, variant="fp16")        
    ...
    def forward(self, x, action_embedding, image_feat=None, text_feature=None, negative_task_emb=None):
        ...
        b, t, c, h, w = x.shape
        image = x[:,0:4].squeeze().clone().detach()
        target = x[:, 4:]
        ...
        if self.model_type == 'i2vgen':
            ...
            target = target.permute(0, 2, 1, 3, 4)
            if target.shape[1] != image.shape[1]: print(target.shape, image.shape)
            if self.use_text: action_embedding = repeat(action_embedding, 'b c -> b f c', f=t-self.horizon)
            
            loss, model_pred = self.pipeline(target, image, action_embedding, image_feat=image_feat)

        elif self.model_type == 'ours-lcd-ll': #TODO
            if image.ndim == 4: image = image.unsqueeze(2)
            elif image.ndim == 5: image = image.permute(0, 2, 1, 3, 4)
            target = target.permute(0, 2, 1, 3, 4)
            if target.shape[1] != image.shape[1]: print(target.shape, image.shape)
...
EPS = 1e-6    
class IndividualHyperPlaneBak(nn.Module):
    def __init__(
        self,
        args, 
        temperature=1.0,
...
EPS = 1e-6    
class GlobalHyperplaneMetricBak(nn.Module):
...
EPS = 1e-6    
class GlobalHyperplaneMetricLayersBak(nn.Module):
    def __init__(
        self,
        args, 
        temperature=1.0,
...
EPS = 1e-6    
class LocalHyperplaneMetricLayersBak(nn.Module):
    def __init__(
        self,
        args, 
        temperature=1.0,
...
class SignalProcess(nn.Module):
    ...
    def forward(self, batch):
        ...
        # use 'text' as a modality 
        if self.use_text and 'text' in batch and len(self.signals) >= 7:
            projected_feature['text'] = repeat(batch['text'], 'b d -> b t d', t=num_frames)
...
class GlobalHyperplaneMetricLayers(nn.Module):
    def __init__(
        ...
        self.linear1 = GlobalActivationLeakyReLULinear(d_input_dim=x_indim, p_input_dim=img_feature_dim, output_dim=output_dim, merge_local=args.merge_local)
        self.linear2 = GlobalActivationLeakyReLULinear(d_input_dim=x_indim, p_input_dim=p_indim, output_dim=output_dim, merge_local=args.merge_local)
        self.linear3 = GlobalActivationLeakyReLULinear(d_input_dim=x_indim, p_input_dim=p_indim, output_dim=output_dim, merge_local=args.merge_local)
    ...
    def forward(self, batch):
        ...
        projected_feature = batch['projected_feature']
        
        
        # pooling over signals max over S [projected feature B x T x S x D -> B x T x D]
        activation, hyperplane = self.linear1(projected_feature, batch['context'], x_res=None, p_res=None, num_frames=batch['num_frames'])
        activation, hyperplane = self.linear2(activation, hyperplane, x_res=projected_feature, p_res=batch['context'], num_frames=batch['num_frames'])
        activation, hyperplane = self.linear3(activation, hyperplane, x_res=projected_feature, p_res=batch['context'], num_frames=batch['num_frames'])
...
class Network(nn.Module):
    def __init__(
        ...
        self.signal_encoder = SignalProcess(args, signals=signals)
        self.hyperplane_layer = GlobalHyperplaneMetricLayers(args)
        
        # self.action_bottleneck_projection = nn.Linear(4*1024, 1024)
        ...
        self.alignment_loss_weight_a = 1e-2
    ...
    def encode_action(self, action_data):
        ...
        if 'context' in action_data: action_data['action_context']['context'] = action_data['context']
        if 'history_context' in action_data: action_data['action_context']['history_context'] = action_data['history_context']
        if 'video_latent' in action_data: action_data['action_context']['video_latent'] = action_data['video_latent']
        if 'text' in action_data: action_data['action_context']['text'] = action_data['text']


        context_features = self.signal_encoder(action_data)
        
        out_dict = self.hyperplane_layer(context_features)

        
        signal_context = out_dict['cond_feature']
        image_context = repeat(action_data['context'], 'b d -> b n d', n=signal_context.shape[1]) # T x B x D
        
        
        history_context_features = self.signal_encoder(action_data['action_context'])
        context_out_dict = self.hyperplane_layer(history_context_features)
        
        
        history_signal_context = context_out_dict['cond_feature'] # B x T x D
        history_image_context = repeat(action_data['history_context'], 'n b d -> b n d', n=history_signal_context.shape[1]) # T x B x D

        # cross attention on history_image_context and history_signal_context 
        history_scores = torch.bmm(history_signal_context, history_image_context.transpose(-2, -1)) / torch.sqrt(torch.tensor(history_image_context.shape[-1], dtype=torch.float32))
        history_attn = history_scores.softmax(dim=-1)
        history_attn_feature = repeat(torch.bmm(history_attn, history_signal_context).mean(1), 'b d -> b t d', t=out_dict['cond_feature'].shape[1])

        scores = torch.bmm(signal_context, image_context.transpose(-2, -1)) / torch.sqrt(torch.tensor(image_context.shape[-1], dtype=torch.float32))
        attn = scores.softmax(dim=-1)
        attn_feature = repeat(torch.bmm(attn, signal_context).mean(1), 'b d -> b t d', t=out_dict['cond_feature'].shape[1])

        signal_feature = torch.cat([attn_feature.unsqueeze(1), history_attn_feature.unsqueeze(1), out_dict['cond_feature'].unsqueeze(1)], dim=1).mean(1)
        
        signal_feature = signal_feature * (torch.rand(signal_feature.shape[0], 1, 1, device = signal_feature.device) > self.cond_drop_chance)
    ...
    def forward(self, x, signal_data, x_dist, text_feature, negative_task_emb=None):
        ...
        # alignment loss
        alignment_loss_d = self.cos_loss(out_dict['p'], text_feature)
        ...
        # pool_feature = pool_feature * (torch.rand(pool_feature.shape[0], 1, 1, device = pool_feature.device) > self.cond_drop_chance)

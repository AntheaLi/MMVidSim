...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        assert model_type in ['avdc', 'i2vgen', 'i2vgen-avdc', 'our', 'ours', 'ours-lcd'], f'{model_type} not implemented'
        ...
        dl = DataLoader(self.ds, batch_size = train_batch_size, shuffle = True, pin_memory = True, num_workers = 32)
        # dl = dataloader
        dl = self.accelerator.prepare(dl)
        self.dl = cycle(dl)
        self.valid_dl = DataLoader(self.valid_ds, batch_size = valid_batch_size, shuffle = False, pin_memory = True, num_workers = 32)
    ...
    def encode_action(self, batch):
        ...
        action = {x: batch['signal'][x][:, 1:] for x in batch['signal'].keys()}
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        ...
                        image_feature = self.image_condition_encoder(batch['video'][:, 0])
                        x_dist = batch['video_dist']
                        loss, model_pred = self.model(x, goal_embed, x_dist, text_feature, other_context=image_feature, negative_task_emb=negative_embed)

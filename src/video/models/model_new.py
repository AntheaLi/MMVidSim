...
class SignalProcess(nn.Module):
    ...
    def forward(self, batch):
        ...
        projected_feature['context'] = batch['context']
...
class GlobalHyperplaneMetricLayers(nn.Module):
    ...
    def forward(self, projected_feature):

        # pooling over signals max over S [projected feature B x T x S x D -> B x T x D]
        activation, hyperplane = self.linear1(projected_feature, projected_feature['context'], x_res=None, p_res=None)
        activation, hyperplane = self.linear2(activation, hyperplane, x_res=projected_feature, p_res=batch['context'])
        activation, hyperplane = self.linear3(activation, hyperplane, x_res=projected_feature, p_res=batch['context'])
...
class Network(nn.Module):
    def __init__(
        ...
        if self.hyper_plane == 'global': self.hyperplane_layers = GlobalHyperplane(args, signals=signals)
        elif self.hyper_plane == 'individual': self.hyperplane_layers = IndividualHyperPlane(args, signals=signals)
        elif self.hyper_plane == 'global-metric-linear': self.hyperplane_layers = GlobalHyperplaneMetric(args, signals=signals)
        elif self.hyper_plane == 'local-metric-mlp': self.hyperplane_layers = LocalHyperplaneMetricLayers(args, signals=signals)
        elif self.hyper_plane == 'global-metric-mlp': self.hyperplane_layers = GlobalHyperplaneMetricLayers(args, signals=signals)
        else:
    ...
    def encode_action(self, action_data):
        ...
        print(history_signal_context.shape)
        history_image_context = rearrange(action_data['history_context'], 'n b d -> b n d') # T x B x D
        print(history_image_context.shape)

        # cross attention on history_image_context and history_signal_context 
        scores = torch.bmm(history_signal_context, history_image_context.transpose(-2, -1)) / torch.sqrt(torch.tensor(history_image_context.shape[-1], dtype=torch.float32))
        attn = scores.softmax(dim=-1)
        attn_feature = repeat(torch.bmm(attn, history_signal_context).mean(1), 'b d -> b t d', t=out_dict['cond_feature'].shape[1])


        encoded_action_features = self.signal_encoder(action_data)

        context_out_dict = self.hyperplane_layers(encoded_action_features)

        out_dict = self.signal_encoder(action_data)

        signal_feature = torch.cat([attn_feature.unsqueeze(1), out_dict['cond_feature'].unsqueeze(1)], dim=1).mean(1)

...
class MemoryEfficientCrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        k = k.unsqueeze(3).reshape(b, k.shape[1], self.heads, self.dim_head).permute(0, 2, 1, 3).reshape(b * self.heads, k.shape[1], self.dim_head).contiguous()
        ...
        v = v.unsqueeze(3).reshape(b, v.shape[1], self.heads, self.dim_head).permute(0, 2, 1, 3).reshape(b * self.heads, v.shape[1], self.dim_head).contiguous()

        # q, k, v = map(
        #     lambda t: t.unsqueeze(3).reshape(b, t.shape[1], self.heads, self.dim_head)
        #     .permute(0, 2, 1, 3)
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head,
        ...
        self.transformer_blocks = nn.ModuleList(
            [BasicTransformerBlock(inner_dim, n_heads, d_head, dropout=dropout, context_dim=context_dim[d], checkpoint=use_checkpoint)
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, context=None):
        xl = x.clone()
        xl = self.attn1(self.norm1(xl), context=context) + xl
        xl = self.attn2(self.norm2(xl), context=context) + xl
        xl = self.ff(self.norm3(xl)) + xl
        return xl
    
    # def forward(self, x, context=None):
    #     xl = x.clone()
    #     x2 = self.attn1(self.norm1(xl), context=context) + xl
    #     x3 = self.attn2(self.norm2(x2), context=context) + x2
    #     x4 = self.ff(self.norm3(x3)) + x3
    #     return x4
...
class TemporalTransformer(nn.Module):
    ...
    """
    def __init__(self, in_channels, n_heads, d_head,
                 depth=1, dropout=0., context_dim=None, frames=16,
                 use_linear=False, use_checkpoint=True, only_self_att=False, multiply_zero=False):
        super().__init__()
        self.multiply_zero = multiply_zero
        self.only_self_att = only_self_att
        self.frames = frames
        self.context_dim = context_dim
        self.only_self_att = False
        ...
        self.transformer_blocks = nn.ModuleList(
            [BasicTransformerBlock(inner_dim, n_heads, d_head, dropout=dropout, context_dim=context_dim[d], checkpoint=use_checkpoint) for d in range(depth)]
        )
        if not use_linear:
    ...
    def forward(self, x, context=None):
        ...
        # [16384, 16, 320]
        if self.use_linear:
            x = rearrange(x, '(b f) c h w -> b (h w) f c', f=f).contiguous()
            x = self.proj_in(x)
        ...
        if self.only_self_att:
            ...
            for i, block in enumerate(self.transformer_blocks):
                ...
                context[i] = rearrange(context[i], '(b f) l con -> b f l con', f=f).contiguous()
                # calculate each batch one by one (since number in shape could not greater then 65,535 for some package)

                # print(f'context i {i}', context[i].shape)
                for j in range(b):
                    # context_i_j = repeat(context[i][j], 'f l con -> (f r) l con', r=(h*w)//self.frames, f=self.frames).contiguous() # this should be 
                    # context_i_j = context[i][j]
                    # context_i_j = repeat(context[i][j], 'f l con -> (f r) l con', r=h*w).contiguous()
                    context_i_j = repeat(context[i][j], 'f l con -> (f r) l con', r=(h*w)//f, f=f).contiguous() # this should be 
                    
#                     print('x[j]', x[j].shape)
#                     print('context_i_j',context_i_j.shape)
                    

                    # context_i_j = context[i][j].unsqueeze(1).repeat(1, (h*w)//self.frames, 1, 1).contiguous()
...
class TemporalTransformer_attemask(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head,
        ...
        if not use_linear:
            ...
            self.proj_in = nn.Linear(in_channels, inner_dim)
            if self.use_adaptor:
                self.adaptor_in = nn.Linear(self.frames, self.frames)
        ...
        if not use_linear:
            ...
            self.proj_out = zero_module(nn.Linear(in_channels, inner_dim))
            if self.use_adaptor:
                self.adaptor_out = nn.Linear(self.frames, self.frames)
        self.use_linear = use_linear
    ...
    def forward(self, x, context=None):
        ...
        if self.only_self_att:
            ...
            for i, block in enumerate(self.transformer_blocks):
                ...
                # calculate each batch one by one (since number in shape could not greater then 65,535 for some package)
                for j in range(b):
                    context_i_j = repeat(context[i][j], 'f l con -> (f r) l con', r=(h*w)//f, f=f).contiguous()
                    x[j] = block(x[j], context=context_i_j)

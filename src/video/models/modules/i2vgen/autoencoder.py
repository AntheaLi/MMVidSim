...
from exp.video.config import i2v_ae_setup
...
class I2VAutoEncoder(AutoencoderVideo):
    def __init__(self,):
        super().__init__(**i2v_ae_setup)

    def forward(self, input, sample_posterior=True):

        b,f,h,w,c = input.shape
        input = rearrange(input, 'b f h w c -> (b f) c h w')

...
from .schedules import beta_schedule_fn
...
class DiffusionDDIM(nn.Module):
    def __init__(self,
        ...
        # assert loss_types in ['mse', 'rescaled_mse', 'kl', 'rescaled_kl', 'l1', 'rescaled_l1','charbonnier']
        ...
        betas = beta_schedule_fn(beta_schedule, **schedule_param)
    ...
    @torch.no_grad()
    def p_sample(self, xt, t, first_frame, task_embed, fps=None, negative_task_emb=None, clamp=None, percentile=None, condition_fn=None, guide_scale=0):
        r"""Sample from p(x_{t-1} | x_t).
            - condition_fn: for classifier-based guidance (guided-diffusion).
            - guide_scale: for classifier-free guidance (glide/dalle-2).
        """
        # predict distribution of p(x_{t-1} | x_t)
        mu, var, log_var, x0 = self.p_mean_variance(xt, t, first_frame, task_embed, fps=fps, negative_task_emb=negative_task_emb, clamp=clamp, percentile=percentile, guide_scale=guide_scale)

        # random sample (with optional conditional function)
        noise = torch.randn_like(xt)
        mask = t.ne(0).float().view(-1, *((1, ) * (xt.ndim - 1)))  # no noise when t == 0
        if condition_fn is not None:
            grad = condition_fn(xt, self._scale_timesteps(t))
            mu = mu.float() + var * grad.float()
        xt_1 = mu + mask * torch.exp(0.5 * log_var) * noise
        return xt_1, x0
    
    @torch.no_grad()
    def p_sample_loop(self, noise, first_frame, task_embed, fps=None, negative_task_emb=None,  clamp=None, percentile=None, condition_fn=None, guide_scale=0):
        ...
        for step in torch.arange(self.num_timesteps).flip(0):
            ...
            xt, _ = self.p_sample(xt, t, first_frame, task_embed, fps=fps, negative_task_emb=negative_task_emb, clamp=clamp, percentile=percentile, condition_fn=condition_fn, guide_scale=guide_scale)
        return xt
    
    def p_mean_variance(self, xt, t, first_frame, task_embed, fps=None, negative_task_emb=None, clamp=None, percentile=None, guide_scale=0):
        r"""Distribution of p(x_{t-1} | x_t).
        """

        # predict distribution
        if guide_scale == 0:
            out = self.model(xt, self._scale_timesteps(t), y=task_embed, local_image=first_frame, fps=fps)
        else:
            # classifier-free guidance
            positive_kwargs = {'y': task_embed,  'local_image':first_frame, 'fps': fps}
            negative_kwargs = {'y': negative_task_emb,  'local_image':first_frame, 'fps': fps}
            # (model_kwargs[0]: conditional kwargs; model_kwargs[1]: non-conditional kwargs)
            y_out = self.model(xt, self._scale_timesteps(t), **positive_kwargs) # positive
            u_out = self.model(xt, self._scale_timesteps(t), **negative_kwargs) # negative
    ...
    @torch.no_grad()
    def ddim_sample(self, xt, t, first_frame, task_embed, fps=None, negative_task_emb=None, clamp=None, percentile=None, guide_scale=0, ddim_timesteps=20, eta=0.0):
        ...
        # predict distribution of p(x_{t-1} | x_t)
        _, _, _, x0 = self.p_mean_variance(xt, t, first_frame, task_embed, fps=fps, negative_task_emb=negative_task_emb, clamp=clamp, percentile=percentile, guide_scale=guide_scale)
    ...
    @torch.no_grad()
    def ddim_sample_loop(self, noise, first_frame, task_embed, fps=None, negative_task_emb=None, clamp=None, percentile=None, guide_scale=0, ddim_timesteps=20, eta=0.0):
        # prepare input
        b = noise.size(0)
        xt = noise
        ...
        for step in steps:
            t = torch.full((b, ), step, dtype=torch.long, device=xt.device)
            xt, _ = self.ddim_sample(xt, t, first_frame, task_embed, fps=fps, negative_task_emb=negative_task_emb, clamp=clamp, percentile=percentile,  guide_scale=guide_scale, ddim_timesteps=ddim_timesteps, eta=eta)
        return xt
    
    @torch.no_grad()
    def ddim_reverse_sample(self, xt, t, first_frame, task_embed, fps,  negative_task_emb=None, clamp=None, percentile=None, guide_scale=0, ddim_timesteps=20):
    ...
    @torch.no_grad()
    def ddim_reverse_sample_loop(self, x0, first_frame, task_embed, fps=None, negative_task_emb=None,  clamp=None, percentile=None, guide_scale=0, ddim_timesteps=20):
        # prepare input
        b = x0.size(0)
        xt = x0
        ...
        for step in steps:
            t = torch.full((b, ), step, dtype=torch.long, device=xt.device)
            xt, _ = self.ddim_reverse_sample(xt, t, first_frame, task_embed, fps=fps, negative_task_emb=negative_task_emb, clamp=clamp, percentile=percentile, guide_scale=guide_scale, ddim_timesteps=ddim_timesteps)
        return xt
    
    @torch.no_grad()
    def plms_sample(self, xt, t, first_frame, task_embed, fps=None, negative_task_emb=None, clamp=None, percentile=None, condition_fn=None, guide_scale=0, plms_timesteps=20):
    ...
    @torch.no_grad()
    def plms_sample_loop(self, noise, first_frame, task_embed, fps=None, negative_task_emb=None, clamp=None, percentile=None, condition_fn=None, guide_scale=0, plms_timesteps=20):
        # prepare input
        b = noise.size(0)
        xt = noise
        ...
        for step in steps:
            # PLMS sampling step
            t = torch.full((b, ), step, dtype=torch.long, device=xt.device)
            xt, _, eps = self.plms_sample(xt, t, first_frame, task_embed, fps=fps, negative_task_emb=negative_task_emb, clamp=clamp, percentile=percentile, condition_fn=condition_fn, guide_scale=guide_scale, plms_timesteps=plms_timesteps, eps_cache=eps_cache)
    ...
    @torch.no_grad()
    def sample(self, first_frame, task_embed, fps=None, negative_task_emb=None, batch_size = 16, guide_scale=0):

        image_size, channels = self.image_size, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample_loop

        noise = torch.randn([1, channels, self.num_frames, image_size[0], image_size[1]])
        noise = noise.to(task_embed.device)

        t  = torch.randint(0, self.num_timesteps, (batch_size,), dtype=torch.long, device=task_embed.device)
        
        return sample_fn(noise, first_frame, task_embed, fps, negative_task_emb=negative_task_emb, guide_scale=guide_scale)

    def loss(self, x0, t, out, xt, model_kwargs={}, noise=None, weight = None, use_div_loss= False):

        noise = self.sample_loss(x0, noise)

        # compute loss
        if self.loss_type in ['kl', 'rescaled_kl']:
            loss, _ = self.variational_lower_bound(x0, xt, t, self.model, model_kwargs)
    ...
    def forward(self, x0, first_frame, task_embed, fps=None, negative_task_emb=None):
        
        b, c, f, h, w, device, img_size = *x0.shape, x0.device, self.image_size
        ...
        if task_embed.ndim == 2: task_embed = task_embed.unsqueeze(1)
        if negative_task_emb is not None and negative_task_emb.ndim == 2: negative_task_emb = negative_task_emb.unsqueeze(1)
        if first_frame.ndim == 4: first_frame = first_frame.unsqueeze(1)

        model_kwargs = {'y': task_embed, 'local_image':first_frame, 'negative_task_emb':negative_task_emb} # 'fps': fps}
        
        model_pred = self.model(xt, self._scale_timesteps(t), **model_kwargs)
        loss = self.loss(x0, t, model_pred, xt, model_kwargs)
        
        return loss, model_pred


    def variational_lower_bound(self, x0, xt, t, first_frame, task_embed, fps=None, negative_task_emb=None, clamp=None, percentile=None):
        # compute groundtruth and predicted distributions
        mu1, _, log_var1 = self.q_posterior_mean_variance(x0, xt, t)
        mu2, _, log_var2, x0 = self.p_mean_variance(xt, t, first_frame, task_embed, fps=fps, negative_task_emb=negative_task_emb, clamp=clamp, percentile=percentile)
    ...
    @torch.no_grad()
    def variational_lower_bound_loop(self, x0, first_frame, task_embed, fps=None, negative_task_emb=None, clamp=None, percentile=None):
        ...
        for step in torch.arange(self.num_timesteps).flip(0):
            ...
            vlb, pred_x0 = self.variational_lower_bound(x0, xt, t, first_frame, task_embed, fps=fps, negative_task_emb=negative_task_emb, clamp=clamp, percentile=percentile)

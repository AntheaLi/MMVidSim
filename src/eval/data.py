...
class ActionSenseVideo(Dataset):
    ...
    def __init__(self,
        ...
        if self.horizon > 1:
            self.resample_len = resample_len - 1 + horizon
    ...
    def __getitem__(self, index:int) -> dict:
        ...
        if self.horizon > 1:
            data_dict['video_dist'] = []
            for j in range(self.horizon):
                data_dict['video_dist'].append(np.array([data_dict['video'][i] - data_dict['video'][j] for i in range(4, data_dict['video'].shape[0])])[np.newaxis, ...])
            data_dict['video_dist'] = np.concatenate(data_dict['video_dist'], axis=0).mean(0)
...
class ActionSenseVideobak(Dataset):
    """ ActionSense dataset class for sensor caption.
    """
    def __init__(self,
                 path="Dataset/",
...
class ActionSenseVideoAblation(Dataset):
    """ ActionSense dataset class for sensor caption.
    """
    def __init__(self,
                 path="Dataset/",
        ...
        if self.horizon > 1:
            self.resample_len = resample_len - 1 + horizon
        self.chosen_signal_keys = parse_signal_keys
        self.resample_type = resample_type
        self.compact = compact
        self.parse_signal = parse_signal
        self.video_type = video_type
        ...
        self.all_signal_keys = ['tactile-glove-left', 'tactile-glove-right', 'myo-emg-left', 'myo-emg-right', 'joint-position', 'right-hand-pose', 'left-hand-pose']
        self.other_signals_keys = [k for k in self.all_signal_keys if k not in self.chosen_signal_keys]
        self.parse_signal_keys = self.all_signal_keys
    ...
    def __getitem__(self, index:int) -> dict:
        ...
        if self.parse_signal:
            signal_dict = {}
            for signal_key in self.all_signal_keys:
                ...
                if signal_key in self.chosen_signal_keys:
                    signal_dict[signal_key] = signal_data
                else:
                    signal_dict[signal_key] = np.zeros(signal_data.shape)

...
from models.model_hyperplane_history_trainer_noatten import Trainer
from models.model_video_history_hyperplane_noatten import VideoDiffusion, Network
...
if __name__ == '__main__':   
    ...
    if 'emg' in prefix: chosen_signals = ['tactile-glove-left', 'tactile-glove-right', 'right-hand-pose', 'left-hand-pose', 'joint-position']
    elif 'hand-force' in prefix: chosen_signals = ['myo-emg-left', 'myo-emg-right', 'right-hand-pose', 'left-hand-pose', 'joint-position']
    elif 'hand-pose' in prefix: chosen_signals = ['myo-emg-left', 'myo-emg-right', 'tactile-glove-left', 'tactile-glove-right', 'joint-position']
    else: chosen_signals = ['myo-emg-left', 'myo-emg-right', 'tactile-glove-left', 'tactile-glove-right', 'right-hand-pose', 'left-hand-pose']
    
    print(f'\n\n evaluating data from {ckpt} at checkpoint {ckpt_number} in {chosen_signals} at sampling timestep {sampling_timestep} ... \n\n' )
    ...
    train_dataset = ActionSenseVideoAblation(path=args.data_path, split='train', horizon=4, parse_signal_keys=chosen_signals, resample_len=args.data_resample_len, video_res=args.image_size)
    val_dataset = ActionSenseVideoAblation(path=args.data_path, split='val', horizon=4, parse_signal_keys=chosen_signals, resample_len=args.data_resample_len, video_res=args.image_size)

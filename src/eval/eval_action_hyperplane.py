...
if __name__ == '__main__':
    ckpt = ''
    ckpt_number = 8 #2,  6, 12, 15, 16
    sampling_timestep = 1000
    
    ckpt = sys.argv[1]  if len(sys.argv) > 1 else 'new-d-open-clip-res-train-max-mlp-2'
    ckpt_number = int(sys.argv[2]) if len(sys.argv) > 2 else 2
    sampling_timestep = int(sys.argv[3]) if len(sys.argv) > 3 else 100
    prefix = sys.argv[4] if len(sys.argv) > 4 else 'hand-pose'
    exp_note = sys.argv[5] if len(sys.argv) > 5 else 'eval'
    ...
    if prefix == 'emg': signals = ['myo-emg-left', 'myo-emg-right']
    elif prefix == 'hand-force': signals = ['tactile-glove-left', 'tactile-glove-right']
    elif prefix == 'hand-pose': signals = ['right-hand-pose', 'left-hand-pose']
    elif prefix == 'joint-pose': signals = ['joint-position']
    else: signals = ['myo-emg-left', 'myo-emg-right', 'tactile-glove-left', 'tactile-glove-right', 'right-hand-pose', 'left-hand-pose', 'joint-position']

...
def train_one_epoch(args, model, train_dataloader, optimizers, tb, epoch):
    model.train()
...
def eval_one_epoch(args, model, val_dataloader, tb, epoch):
    model.eval()
...
if __name__ == '__main__':
    ...
    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=min(args.batch_size, len(val_dataset)-1), shuffle=False, num_workers=0, drop_last=True)
    ...
    # start training
    best_loss = eval_one_epoch(args, model=network, val_dataloader=val_dataloader, tb=val_writer, epoch=0)
    ...
    for epoch in range(args.epochs):
        ...
        train_one_epoch(args, model=network, train_dataloader=train_dataloader,  optimizers=optimizers, tb=train_writer, epoch=epoch)

        if epoch % args.eval_freq == 0:
            test_loss = eval_one_epoch(args, model=network, val_dataloader=val_dataloader, tb=val_writer, epoch=epoch)
